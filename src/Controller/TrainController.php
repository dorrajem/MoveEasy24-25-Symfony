...
class TrainController extends AbstractController
{
    ...
    public function index(Request $request, TrainRepository $trainRepository): Response
    {
        $minCapacite = $request->query->get('minCapacite', 0);
        $maxCapacite = $request->query->get('maxCapacite', 4000);
    
        $sort = $request->query->get('sort', 'capacite');
        $direction = strtoupper($request->query->get('direction', 'ASC'));
    
        if (!in_array($sort, ['capacite'])) {
            $sort = 'capacite';
        }
        ...
        $trains = $trainRepository->createQueryBuilder('t')
            ->where('t.capacite BETWEEN :minCapacite AND :maxCapacite')
