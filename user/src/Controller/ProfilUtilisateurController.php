...
class ProfilUtilisateurController extends AbstractController {
    ...
    public function new(Request $request, EntityManagerInterface $em, UtilisateurRepository $userRepo, ValidatorInterface $validator): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            // Validation supplémentaire côté serveur si nécessaire
            $errors = $validator->validate($profil);
            
            if (count($errors) > 0) {
                // Si des erreurs de validation existent, ajoute-les aux erreurs du formulaire
            ...
            if ($telephone !== null) {
                // Normalisation du téléphone (supprime les espaces, etc.)
    ...
    public function edit(Request $request, ProfilUtilisateur $profil, EntityManagerInterface $em, ValidatorInterface $validator): Response
    {
        $form = $this->createForm(ProfilUtilisateurType::class, $profil);
        $form->handleRequest($request);
